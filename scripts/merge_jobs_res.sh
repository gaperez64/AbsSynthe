...
do
	...
	for solver in $(seq 0 1 3)
	do
     		echo "============  $file_name.aag =============" 1>> ${RES_DIR}"/bench_"${solver}".out"
		cat ${RES_DIR}"/bench.out-"${jobid} >> ${RES_DIR}"/bench_"${solver}".out"
		jobid=$(($jobid+1))
	done
done
