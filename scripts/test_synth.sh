...
CALL_SYNTH_TOOL="../AbsSynthe/binary/abssynthe -v L $@ "
TIMESTAMP=`date +%s`
...
do
     ...
     ${GNU_TIME} --output=${RES_TXT_FILE} -a -f "Synthesis time: %e sec (Real time) / %U sec (User CPU time)" ${CALL_SYNTH_TOOL} $infile_path >> ${RES_TXT_FILE} "-o" $outfile_path >> ${RES_TXT_FILE}
     exit_code=$?
