...
static vector<pair<unsigned, BDD>>
synthAlgo(Cudd *mgr, BDDAIG *spec, BDD non_det_strategy, BDD care_set) {
    ...
    for (vector<aiger_symbol *>::iterator i = c_inputs.begin();
        ...
        // dbgMsg("Can be true BDD size: " +
        // to_string(can_be_true.nodeCount()));
        //  pairs (x,u) in which c can be false
        BDD can_be_false = c_arena.Cofactor(~c);
        // dbgMsg("Can be false BDD size: " +
        // to_string(can_be_false.nodeCount()));
        BDD must_be_true = (~can_be_false) & can_be_true;
        // dbgMsg("Must be true BDD size: " +
        // to_string(must_be_true.nodeCount()));
        BDD must_be_false = (~can_be_true) & can_be_false;
        // dbgMsg("Must be false BDD size: " +
        // to_string(must_be_false.nodeCount()));
...
static void
finalizeSynth(Cudd *mgr, AIG *spec,
    ...
    // NOTE: because of the way bdd2aig is implemented, we must ensure that BDDs
    // are no longer operated on after this point!
    unordered_map<unsigned long, unsigned> cache;
    ...
    for (vector<unsigned>::iterator i = cinputs.begin(); i != cinputs.end();
         i++) {
...
static void outputWinRegion(Cudd *mgr, BDDAIG *spec, BDD winning_region) {
    ...
    blank_spec.addOutput(bdd2aig(mgr, &blank_spec, winning_region, &cache),
                         "winning region");
...
static void outputIndCertificate(Cudd *mgr, BDDAIG *spec, BDD winning_region) {
    ...
    for (vector<aiger_symbol *>::iterator i = latches.begin();
        ...
        unsigned var =
            blank_spec.copyGateFromAux(spec, (*i)->next, &copy_cache);
    ...
    BDD primed_winning_region =
        simple_winning_region.SwapVariables(clean_original_latch, latch_next);
#ifndef NDEBUG
    ...
    unsigned w_primed =
        bdd2aig(mgr, &blank_spec, primed_winning_region, &cache);
    dbgMsg("W' = " + to_string(w_primed));
    // we want W => W'(L<-f_l) so we will do ~(w & ~w')
    blank_spec.addOutput(
        AIG::negateLit(blank_spec.optimizedGate(w, AIG::negateLit(w_primed))),
        "inductivity check");
...
static BDD substituteLatchesNext(BDDAIG *spec, BDD dst,
    ...
    if (settings.use_trans) {
        ...
        result = trans_rel_bdd.AndAbstract(primed_dst, primed_latch_cube);
    } else {
...
static BDD abstractSafeCpreAux(BDDAIG *spec, BDD safe, BDD untracked_actions,
    ...
    BDD uinput_tracked_cube =
        spec->uinputCube().ExistAbstract(untracked_actions);
    return temp_bdd.UnivAbstract(uinput_tracked_cube);
}

static BDD abstractSafeCpre(BDDAIG *spec, BDD safe, BDD untracked_latches,
                            BDD untracked_actions, BDD &cache_trans_bdd,
...
static bool internalSolveAbstract(Cudd *mgr, BDDAIG *spec, const BDD *cpre_init,
    ...
    while (cont) {
        ...
        while (cont2 && !includes_init) {
            ...
            BDD res = abstractSafeCpre(spec, mayWin, untracked_latches,
                                       untracked_actions, cache_trans,
                                       cache_non_absd_result);
            BDD mayWin1 = mayWin;
            mayWin = mayWin & res;
            if (mayWin1 == mayWin) {
                cont2 = false;
        ...
        cache =
            ~abstractSafeCpreAux(spec, mayWin, untracked_actions, cache_trans);
        ...
        if (includes_init || mayLose.IsZero())
            ...
            untracked_latches =
                untracked_latches.ExistAbstract(implicant.Support());
            BDD new_untracked =
                untracked_actions.ExistAbstract(implicant.Support());
    ...
    dbgMsg("Early exit? " + to_string(includes_init) + ", after " +
           to_string(cnt) + " iterations.");

    if (!includes_init && do_synth && outputExpected()) {
        dbgMsg("acquiring lock on synth mutex");
        if (data != NULL)
            pthread_mutex_lock(&data->synth_mutex);
    ...
    while ((!(init_state & mayWin).IsZero()) && (!fixpoint)) {
        ...
        if (mayWin.nodeCount() < threshold) {
            ...
            dbgMsg("Algos.cpp: Size < Threshold ; Fixpoint step " +
                   to_string(++cnt) +
                   " Bdd size : " + to_string(mayWin.nodeCount()));
            BDD mayWin1 =
                mayWin & abstractSafeCpre(spec, mayWin, mgr->bddOne(),
            ...
            BDD mayWin1 =
                mayWin & abstractSafeCpre(spec, mayWin, untracked_latches,
                                          mgr->bddOne(), cache_trans_bdd,
                                          cache_non_abstracted_result);
            if (mayWin1 == mayWin) {
                dbgMsg("Promoting");
...
static bool internalSolveExact(Cudd *mgr, BDDAIG *spec, const BDD *upre_init,
    ...
    if (!includes_init && do_synth && outputExpected()) {
        ...
        BDD clean_winning_region =
            (~error_states).Cofactor(~spec->errorStates());
#ifndef NDEBUG
...
static bool internalSolve(Cudd *mgr, BDDAIG *spec, const BDD *upre_init,
    ...
    // set
    if (settings.use_abs && (losing_region == NULL) &&
        ...
        if (!settings.abs_threshold)
            ...
            return internalSolveAbstractBackAndForth(mgr, spec, upre_init,
                                                     do_synth);
    }
...
static bool compSolve1(Cudd *mgr, BDDAIG *spec) {
    ...
    for (vector<BDDAIG *>::iterator i = subgames.begin(); i != subgames.end();
         i++) {
        gamecount++;
        BDD error_states;
        BDD bad_transitions;
        ...
        if (!internalSolve(mgr, *i, NULL, &error_states, &bad_transitions)) {
            ...
            subgame_results.push_back(
                pair<BDD, BDD>(error_states, bad_transitions));
        }
    }
    ...
    if (!cinput_independent) { // we still have one game to solve
        ...
        for (vector<BDDAIG *>::iterator i = subgames.begin();
            ...
            all_cinput_strats.insert(all_cinput_strats.end(), temp.begin(),
                                     temp.end());
            sg++;
            delete *i;
        }
        BDD clean_winning_region =
            (~global_lose).Cofactor(~spec->errorStates());
        // let us clean the AIG before we start introducing new stuff
        spec->popErrorLatch();
        if (settings.out_file != NULL) {
...
static bool compSolve2(Cudd *mgr, BDDAIG *spec) {
    ...
    while (subgame_results.size() >= 2) {
        ...
        list<subgame_info>::iterator min_it,
            min_jt;                  // iterators to selected games
        ...
        int i, j;
        for (i = 0, it = subgame_results.begin(); it != subgame_results.end();
             i++, it++) {
            jt = it;
            jt++;
            j = i + 1;
        ...
        dbgMsg("Selected subgames " + to_string(min_i) + " and " +
               to_string(min_j));
        set<unsigned> intersection;
    ...
    if (outputExpected()) {
        ...
        clean_winning_region = clean_winning_region.ExistAbstract(
            spec->cinputCube() & spec->uinputCube());
        // let us clean the AIG before we start introducing new stuff
        spec->popErrorLatch();
        if (settings.out_file != NULL) {
            dbgMsg("Starting synthesis");
            synth_data.c_functions = synthAlgo(
                mgr, spec, ~subgame_results.back().first, mgr->bddOne());
        }
...
static bool compSolve3(Cudd *mgr, BDDAIG *spec) {
    ...
    for (unsigned i = 0; i < subgames.size(); i++) {
        ...
        if (!internalSolve(mgr, subgames[i], NULL, &losing_region,
            ...
            return false;
        subgame_results.push_back(
            subgame_info(~losing_region, ~losing_transitions));
    ...
    while (!includes_init && prev_lose != global_lose) {
        ...
        for (unsigned i = 0; i < subgames.size(); i++) {
            ...
            BDD local_lose =
                global_lose.UnivAbstract(spec->toCube(rem_latches));
        ...
        if (global_lose == prev_lose) {
            dbgMsg("Global Upre");
            global_lose =
                global_lose | upre(spec, global_lose, global_losing_trans);
            addTime("globalstep");
        } else {
...
static bool compSolve4(Cudd *mgr, BDDAIG *spec) {
    ...
    for (unsigned i = 0; i < subgames.size(); i++) {
        ...
        if (!internalSolve(mgr, subgames[i], NULL, &losing_region,
                           &losing_transitions))
            return false;
        subgame_results.push_back(subgame_info(
            ~losing_region, (~losing_transitions & ~losing_region)));
    ...
    while (something_changed) {
        ...
        for (unsigned i = 0; i < subgames.size(); i++) {
            ...
            if ((winning_strats & global_win_strats) != winning_strats) {
                ...
                set<unsigned> latches_sg =
                    spec->getBddLatchDeps(sg_info.second);
                set<unsigned> rem_latches;
                set_difference(latches_u.begin(), latches_u.end(),
                               latches_sg.begin(), latches_sg.end(),
                               inserter(rem_latches, rem_latches.begin()));
                ...
                if (nu_local_win_strats != winning_strats) {
                    ...
                    assert((~nu_local_win_strats | ~winning_states) ==
                           ~nu_local_win_strats);
                    assert((~nu_local_win_strats | winning_strats) ==
                           mgr->bddOne());
